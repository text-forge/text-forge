...
func _editor_notification(type: int, title: String, text: String) -> void:
	...
	if not (get_parent().current_tab == index and visible):
		Global.get_panel_manager().change_panel_icon(place, index, U.load_resource("res://core/panels/notifications/notification.png"))
	if not mute:
...
func _on_tab_changed() -> void:
	if get_parent().current_tab == index and visible:
		Global.get_panel_manager().change_panel_icon(place, index, U.load_resource("res://core/panels/notifications/icon.png"))
...
func _on_mute_toggled(toggled_on: bool) -> void:
	if toggled_on:
		mute_bottom.icon = U.load_resource("res://core/panels/notifications/mute.png")
	else:
		mute_bottom.icon = U.load_resource("res://core/panels/notifications/icon.png")
	mute = toggled_on
